...
I am also involved in other studies in which we evaluated the effect of placing three cameras per sampling point (not in the same tree, but in the surrounding area) on detection rates. We also evaluated the effect of moving camera locations instead of keeping them fixed for the whole study period, etc. Briefly, we demonstrated that clustered designs (three cameras per sampling point) resulted in an increase in the precision of detection rates compared to single designs (one camera per sampling point) when a fixed number of sampling points were monitored. When a fixed number of cameras were available, rotating the cameras to new independent sampling points improved precision when monitoring aggregated populations. [More here!](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/1365-2664.14690). We used simulations for this kind of work.
</div>
<br/>
![](images/simpoints_web.jpg){width=90%}
<figcaption style="font-size: 0.85em; color: #082a64;">
Two point patterns simulated, each point represenint a location where an animal was. On the left side, circular home-ranges with half-normal decay in the number of points (proxy of animals with  clear home range center, for instance those species using burrows). On the right side, trail-based distribution (proxy of animals that frequently moves along trails, such as wolves or foxes).
</figcaption>
<br/>
...
I frequently use **simulations** to evaluate different aspects related to methods reliability and precision. Working with simulations allow me to "know the truth" (i.e. true population density), something not fleaxible in wild populations. Simulations allow me to better understand the impact of population and/or design characteristics. Regarding population, I usually simulate diferent leves of underlaying abundance, aggregation, movement patterns, etc. Regarding design, camera trap spacing, number of cameras, detection performance, etc what I usually play with.
</div>
<br/>
![](images/SimulationGIF.gif){width=70%}
<figcaption style="font-size: 0.85em; color: #082a64;">
Visualization of a simulated population sampled with camera traps. The GIF shows two animals (red & blue) moving arround a regular grid of cameras (white points). The background green layer represents a simulated habitat suitability.
</figcaption>
